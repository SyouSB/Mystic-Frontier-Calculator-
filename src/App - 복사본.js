...
const App = () => {
  ...
  const analyzeFrame = async () => {
              ...
              if (isSiteCategory) {
                ...
                if (!isInSiteROI) {
                    diceCandidates.push({
                        id: tmpl.id,
                        score: score,
                        x: col, y: row, w: resizedTmpl.cols, h: resizedTmpl.rows,
                        usedScale: s
                    });
                }
              }
    ...
    const finalSites = detectedSites.map(site => {
      ...
      return { ...site, id: newId, hue: hue, sat: sat };
    });    
  ...
  // 1. 텍스트 전처리: 불필요한 노이즈 제거 및 줄바꿈 정리
    ...
    // 2. "If"를 기준으로 문장을 쪼개어 분석 (여러 조건 대응)
    const sentences = normalizedText.split(/If/i);
    ...
    sentences.forEach(sentence => {
      ...
      if (totalMatch || multiMatch) {
        ...
        // 조건 텍스트 정제 (끝의 콜론, 쉼표 등 제거)
        const conditionText = trimmed.substring(0, effectIndex).replace(/[:;.,-]$/, "").trim();

        parsedEffects.push({
          text: "If " + trimmed,
          condition: conditionText, // 추출된 조건절
        ...
        <div style={{ flex: 1, background: '#252525', padding: '15px', borderRadius: '8px', overflowY: 'auto', maxHeight: '720px' }}>
          ...
          {analysisResult.siteDetails.length > 0 ? analysisResult.siteDetails.map((s, i) => (
            <div key={i} style={{ fontSize: '12px', marginBottom: '8px', padding: '8px', background: '#333', borderRadius: '4px', borderLeft: '4px solid #00bfff' }}>
              ...
              <span style={{ color: '#ffeb3b' }}>Hue: {s.hue} / Sat: {s.sat}</span>
            </div>
          )) : <p style={{ fontSize: '12px', color: '#888' }}>Empty</p>}
              ...
              analysisResult.parsedEffects.map((eff, i) => (
                  <div key={i} style={{ 
                      ...
                      <div style={{fontSize: '11px', color: eff.isActive ? '#fff' : '#aaa'}}>
                        {eff.condition ? `If ${eff.condition}` : '[Passive] Always Active'}
                      </div>
